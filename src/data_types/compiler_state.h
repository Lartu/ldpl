...
struct compiler_state {
  ...
  // 0 no section, 1 data or local, 2 procedure, 3 sub-procedure start, 4
  // parameters Code to output (plain C code)
  code_location where = {"", 0};
  vector<string> variable_code;
  vector<string> output_code;
  vector<string> subroutine_code;  // code outside main()
  // variables
  map<string, map<string, vector<unsigned int>>>
  ...
  void add_code(string code) {
    auto& output =
        current_subprocedure == "" ? this->output_code : this->subroutine_code;
  ...
  void add_code(string code, code_location where) {
    ...
    // if (line_num)
    //    output.push_back("#line " + to_string(where.current_line) + " \"" +
    //    escape_c_quotes(where.current_file) + "\"");
    output.push_back(code);
  }
  ...
  void open_if() { block_stack.push(1); }
  void close_if() { block_stack.pop(); }
  bool closing_if() { return !block_stack.empty() && block_stack.top() == 1; }
  void open_else() { block_stack.top() = 3; }
  bool closing_else() { return !block_stack.empty() && block_stack.top() == 3; }
  void open_loop() {
    ++open_loops;
    block_stack.push(2);
  }
  ...
  // an error is risen.
  vector<pair<string, vector<vector<unsigned int>>>>
      expected_subprocedures;  // subprocedure -> list of parameter types
  void add_expected_subprocedure(string name, string fixed_name,
                                 vector<vector<unsigned int>>& types) {
    for (auto& subprocedure : expected_subprocedures)
      if (subprocedure.first == name) return;
  ...
  void remove_expected_subprocedure(string& name) {
    for (auto it = expected_subprocedures.begin();
         it != expected_subprocedures.end(); ++it) {
      if (it->first == name) {
  ...
  bool correct_subprocedure_types(string& name,
                                  vector<vector<unsigned int>>& types) {
    for (auto& subprocedure : expected_subprocedures)
      if (subprocedure.first == name) return types == subprocedure.second;
    for (auto& subprocedure : subprocedures)
      if (subprocedure.first == name) {
        vector<vector<unsigned int>> actual_types;
        for (auto& parameter : subprocedure.second)
  ...
  vector<pair<string, string>>
      custom_statements;  // (statement model line, subprocedure name)
