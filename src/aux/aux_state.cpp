/* This file contains auxiliary functions that check the current compilation state */
...
{
    // -- Returns is an identifier is a valid scalar variable or an access that results in one --
    return is_num_var(token, state) || is_txt_var(token, state);
}

bool is_num_expr(string &token, compiler_state &state)
{
    // -- Returns is an identifier is a valid scalar variable or number or an access that results in one --
    return is_num_var(token, state) || is_number(token);
}

bool is_txt_expr(string &token, compiler_state &state)
{
    // -- Returns is an identifier is a valid scalar variable or text or an access that results in one --
    return is_txt_var(token, state) || is_string(token);
}

bool is_expression(string &token, compiler_state &state)
{
    // -- Returns is an identifier is a valid scalar variable or text or number or an access that results in one --
...
bool variable_exists(string &token, compiler_state &state)
{
    ...
    // (Bear in mind that myList is a variable, myList:0 is not, that's an access for all this function is concerned)
...
vector<unsigned int> variable_type(string &token, compiler_state &state)
{
    ...
    //
    // Return the number of the type or {0} if the variable doesn't exist. This function can
    // take full variables (foo:0:"hi"). Returns all the types of the variable.
    // If foo is number map list {1, 4, 3} and we just pass it foo:0, it will return {1, 4},
    // that is the type foo:0 has.
    //
