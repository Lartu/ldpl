...
bool line_like(string model_line, vector<string> &tokens, compiler_state &state)
{
    ...
    for (; i < model_tokens.size(); ++i)
    {
        ...
        {
            return is_num_map(tokens[j], state);
        }
        else if (model_tokens[i] == "$str-vec") // $str-vec is TEXT vector
        {
            return is_txt_map(tokens[j], state);
        }
        else if (model_tokens[i] == "$list")
        { // $list is a LIST
            return variable_type(tokens[j], state).back() == 3;
        }
        else if (model_tokens[i] == "$map")
        { // $map is a MAP
            return variable_type(tokens[j], state).back() == 4;
        }
        else if (model_tokens[i] == "$scalar-list")
        { // $scalar-list is a LIST of scalar values
            return is_scalar_list(tokens[j], state);
        }
        else if (model_tokens[i] == "$num-list") // $num-vec is NUMBER list
        {
            return is_num_list(tokens[j], state);
        }
        else if (model_tokens[i] == "$str-list") // $str-vec is TEXT list
        {
            return is_txt_list(tokens[j], state);
        }
        else if (model_tokens[i] == "$list-list") // $str-vec is a LIST of LISTs
        {
            return is_list_list(tokens[j], state);
        }
        else if (model_tokens[i] == "$map-list") // $str-vec is LIST of MAPs
        {
            return is_map_list(tokens[j], state);
        }
        ...
        else if (model_tokens[i].find("$var-type-") == 0) // variable with a given type number
        {
            ...
            {
                return (int)actual_type[t] == expected_type[t] - '0';
            }
        }
        else if (model_tokens[i] == "$natural") // $natural is an integer greater than 0
        {
            return is_natural(tokens[j]);
        }
