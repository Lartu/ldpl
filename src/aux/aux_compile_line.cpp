...
// C++ Code
    ...
    if (line_like("GET LENGTH OF $str-expr IN $num-var", tokens, state))
    {
        ...
        // C++ Code
        state.add_code(get_c_variable(state, tokens[5]) + " = countGraphemes(" + get_c_expression(state, tokens[3]) + ");", state.where);
        return;
    }
                  ...
                  tokens, state))
    {
        ...
        // C++ Code
        state.add_code(
            get_c_variable(state, tokens[7]) + " = str_replace(" + get_c_expression(state, tokens[5]) + ", " + get_c_expression(state, tokens[1]) + ", " + get_c_expression(state, tokens[3]) + ");",
            state.where);
        return;
    }
    // deprecated
                  ...
                  tokens, state))
    {
        ...
        // C++ Code
        state.add_code(
            get_c_variable(state, tokens[7]) + " = str_replace(" + get_c_expression(state, tokens[3]) + ", " + get_c_expression(state, tokens[1]) + ", " + get_c_expression(state, tokens[5]) + ");",
            state.where);
        return;
    }
