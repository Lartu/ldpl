...
void compile_line(vector<string> &tokens, compiler_state &state) {
                ...
                state))  // If it's a variable declaration
  {
    ...
    if (tokens[i] == "NUMBER") {  // If it's a number...
      ...
      if (extern_keyword == "")
        assign_default = " = 0";  // if its not an external variable, set a
               ...
               "TEXT") {         // If we are dealing with a text variable...
      ...
      while (valid_type && i < tokens.size()) {
        if (i % 2 == 1) {
          if (tokens[i] != "OF") valid_type = false;
        } else if (i % 2 == 0) {
    ...
    while (valid_type &&
           i < tokens.size()) {  // If up to this point we got a valid data
                                 // type, we check for containers.
      assign_default =
          "";  // Collections are not initialized with any default values.
      if (tokens[i] == "MAP" ||
          tokens[i] == "VECTOR") {  // If its a MAP (aka VECTOR)
  ...
  if (line_like("ELSE IF $condition THEN", tokens, state)) {
    ...
    if (condition != "[ERROR]") {
      ...
      if (!state.closing_if()) badcode("ELSE IF without IF", state.where);
      // C++ Code
  ...
  if (line_like("ELSE", tokens, state)) {
    ...
    if (!state.closing_if()) badcode("ELSE without IF", state.where);
    // C++ Code
    state.open_else();
  ...
  if (line_like("FOR $num-var FROM $num-expr TO $num-expr STEP $num-expr DO",
                tokens, state)) {
    if (!in_procedure_section(state))
...
// C++ Code
  ...
  // Text Statements
  if (line_like("JOIN $expression AND $expression IN $str-var", tokens,
                state)) {
    if (!in_procedure_section(state))
      badcode("JOIN statement outside PROCEDURE section", state.where);
    // C++ Code
    state.add_code("join(" + get_c_string(state, tokens[1]) + ", " +
                       get_c_string(state, tokens[3]) + ", " +
  ...
  if (line_like("GET LENGTH OF $str-expr IN $num-var", tokens, state)) {
    ...
    // C++ Code
    state.add_code(get_c_variable(state, tokens[5]) + " = ((chText)" +
  ...
  if (line_like("REPLACE $str-expr WITH $str-expr FROM $str-expr IN $str-var",
    ...
    // C++ Code
    state.add_code(
        get_c_variable(state, tokens[7]) + " = str_replace(((chText)" +
            get_c_expression(state, tokens[5]) + ").str_rep(), ((chText)" +
  ...
  if (line_like("REPLACE $str-expr FROM $str-expr WITH $str-expr IN $str-var",
    ...
    state.add_code(
        get_c_variable(state, tokens[7]) + " = str_replace(((chText)" +
            get_c_expression(state, tokens[3]) + ").str_rep(), ((chText)" +
            get_c_expression(state, tokens[1]) + ").str_rep(), ((chText)" +
  ...
  if (line_like("SPLIT $str-expr BY $str-expr IN $str-list", tokens, state)) {
    ...
    // C++ Code
    state.add_code(get_c_variable(state, tokens[5]) + " = utf8_split_list(" +
                       get_c_expression(state, tokens[1]) + ", " +
                       get_c_expression(state, tokens[3]) + ");",
                   state.where);
    return;
  ...
  for (pair<string, string> &statement : state.custom_statements) {
    if (line_like(statement.first, tokens, state)) {
      ...
      for (size_t i = 0; i < model_tokens.size(); i++) {
        if (model_tokens[i][0] == '$') parameters.push_back(tokens[i]);
      }
